layui.config({
  	...
  	,jPages: 'jPages/jPages.min'
  	,emoticons: 'emoticons/jquery.emoticons'
...
layui.use(['util','element','form'], function(){
	...
	//统计输入的文字数
	.on('change keydown keyup','textarea[maxlength]', function(event) {
		...
		$('[data-id="'+$(this).data('rel-txt')+'"]').text(maxlength-val.length);
		}
	});
});
...
;(function($,window,document){
	window.JPlaceHolder = {//处理低版IE6,7,8,9的html5 的 placeholder 兼容问题，用法 JPlaceHolder.init();
	    //检测
	    _check : function(){
	    ...
	    //修复
	    fix : function(){
	        jQuery(':input[placeholder]:visible').each(function(index, element) {
				var self = $(this), txt = self.attr('placeholder');
				var val = self.val();
				if(val==""||val==undefined){
					if(!$(this).parent().hasClass("tips_par_box")){
						self.wrap($('<div class="tips_par_box"></div>').css({position:'relative', zoom:'1', border:'none', background:'none', padding:'none', margin:'none'}));
						var pos = self.position(), h = self.outerHeight(true), paddingleft = self.css('padding-left');
						var holder = $('<span class="p_tips"></span>').text(txt).css({position:'absolute', left:pos.left, top:pos.top, height:h, "line-height":h+"px", paddingLeft:paddingleft, color:'#999'}).appendTo(self.parent());
